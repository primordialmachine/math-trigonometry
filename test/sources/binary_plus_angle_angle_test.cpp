...
TEST(trigonometry_tests, binary_plus_angle_angle_tests)
{
  using degrees = angle<angle_unit_degrees, float>;
  ASSERT_TRUE(degrees(3.f) == degrees(1.f) + degrees(2.f));
  using radians = angle<angle_unit_radians, float>;
  ASSERT_TRUE(radians(3.f) == radians(1.f) + radians(2.f));
  using turns = angle<angle_unit_turns, float>;
  ASSERT_TRUE(turns(3.f) == turns(1.f) + turns(2.f));
}
