...
TEST(trigonometry_tests, binary_star_scalar_angle_tests)
{
  using degrees = angle<angle_unit_degrees, float>;
  ASSERT_TRUE(degrees(9.f) == 3.f * degrees(3.f));
  using radians = angle<angle_unit_radians, float>;
  ASSERT_TRUE(radians(9.f) == 3.f * radians(3.f));
  using turns = angle<angle_unit_turns, float>;
  ASSERT_TRUE(turns(9.f) == 3.f * turns(3.f));
}
