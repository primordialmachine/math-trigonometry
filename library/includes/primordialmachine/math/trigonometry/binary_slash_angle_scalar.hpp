...
template<typename A, typename B>
  struct binary_slash_functor < A,
  B,
  enable_if_t<
    conjunction_v<is_angle<A>, is_scalar<B>, is_same<underlying_type_t<A>, B>>>>
{
  ...
  result_type operator()(const left_operand_type& left_operand,
    ...
    noexcept(noexcept(result_type{ left_operand.value() / right_operand }))
  {
    return result_type{ left_operand.value() / right_operand };
  }
