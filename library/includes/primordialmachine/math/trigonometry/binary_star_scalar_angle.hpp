...
struct binary_star_functor<
  ...
  B,
  enable_if_t<
    conjunction_v<is_scalar<A>, is_angle<B>, is_same<A, underlying_type_t<B>>>>>
{
  ...
  result_type operator()(const left_operand_type& left_operand,
    ...
    noexcept(noexcept(result_type{ right_operand.value() }))
  {
    return result_type{ left_operand * right_operand.value() };
  }
}; // struct binary_star_functor
